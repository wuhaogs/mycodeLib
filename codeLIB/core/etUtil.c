...
void lstFree2
    ...
    VOIDFUNCPTR    freeFunc        /* free() function to call */
    )
    {
    NODE *p1, *p2;
...
EUINT32 EtRand (void) 
{
    ...
    return (EUINT32) (randSeed/65536) % (ERAND_MAX + 1);
}
...
void *EtSrand 
(
 EUINT32 seed	    /* random number seed */
) 
{
    randSeed = seed;
    return (void *)0;
...
/*getns get n char from std input*/

char *getns(char *buf, int n)
{
    int c, i = 0;
    char *s;
    for (s = buf; ((c = getchar()) != '\n') && (++i < n);)
